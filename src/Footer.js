import React from "react";

export default function Footer() {
  return (
    <div className="Footer">
      <h3>
        <a href="https://github.com/amberleighpurnis/weather-react">
          {" "}
          Open-source code{" "}
        </a>{" "}
        by Amber Leigh Purnis{" "}
        <a
          href="https://www.linkedin.com/in/amberleighpurnis"
          target="_blank"
          rel="noopener noreferrer"
        >
          {" "}
          <img
            className="connectLinks"
            alt={"linkedin Icon"}
            src={
              "data:image/png;base64,"
            }
          />
        </a>{" "}
        <a
          href="https://www.instagram.com/amberl3igh/"
          target="_blank"
          rel="noopener noreferrer"
        >
          {" "}
          <img
            className="connectLinks"
            alt={"instagram Icon"}
            src={
              "https://i.pinimg.com/564x/a2/5f/4f/a25f4f58938bbe61357ebca42d23866f.jpg"
            }
          />
        </a>{" "}
      </h3>
    </div>
  );
}
